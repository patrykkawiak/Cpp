...
int main() {
    ...
    if (std::toupper(multiChoice) == 'Y') {
        ...
        auto bestPlayer = std::min_element(players.begin(), players.end(),
                                           [](const Player& p1, const Player& p2) { return p1.getBestScore() < p2.getBestScore(); });

        if (bestPlayer != players.end()) {
            std::cout << "The winner is: " << bestPlayer->getNickname() << " with " << bestPlayer->getBestScore() << " attempts!" << std::endl;
        } else {
